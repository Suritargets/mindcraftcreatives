...
type SpecItem = { label: string; values: string[] };
  ...
  function handleSave() {
    startTransition(async () => {
      await createProduct({
        ...
        metaTitle,
        metaDescription,
        specs: specs.map((s) => ({ label: s.label, values: s.values })),
      });
      setSaved(true);
      setTimeout(() => {
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Nieuw Product</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Voeg een nieuw product toe aan de catalogus.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleSave} disabled={isPending} className="gap-1.5">
            ...
            {isPending ? "Aanmaken..." : "Product Aanmaken"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-4">
        ...
        <div className="space-y-3">
          ...
          <CollapsibleCard title="Algemene Informatie" defaultOpen={true}>
            <div className="space-y-3">
              <div>
                ...
                <Input value={name} onChange={(e) => handleNameChange(e.target.value)} className="h-9" placeholder="bijv. Custom T-Shirt Bedrukt" />
              </div>
              <div>
                <label className="block text-xs font-medium text-foreground mb-1">Beschrijving</label>
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="text-sm" placeholder="Beschrijf het product..." />
              </div>
              <div>
          ...
          <CollapsibleCard title="Bedrukkingsmethoden" badge={printMethods.length > 0 ? `${printMethods.length}` : undefined} defaultOpen={false}>
            <div className="space-y-2">
              <div className="flex justify-end">
          ...
          <CollapsibleCard title="Voordelen" badge={advantages.length > 0 ? `${advantages.length}` : undefined} defaultOpen={false}>
            <div className="space-y-2">
              <div className="flex justify-end">
          ...
          <CollapsibleCard title="SEO & Meta" variant="sidebar" defaultOpen={false}>
            <div className="space-y-3">
              <div>
                ...
                <Input value={slug} onChange={(e) => setSlug(e.target.value)} className="h-8 text-xs font-mono" placeholder="product-naam" />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Meta Titel</label>
                <Input value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} className="h-8 text-xs" placeholder="Productnaam | Mindcraft Creatives" />
              ...
              <div>
                ...
                <Textarea value={metaDescription} onChange={(e) => setMetaDescription(e.target.value)} rows={3} className="text-xs" placeholder="Korte beschrijving voor zoekmachines..." />
                <div className="mt-1 flex justify-end">
