...
export default function MenuPage() {
  const [mainMenu, setMainMenu] = useState(defaultMenu);
    ...
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Beheer de navigatie van de website.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card>
            <div className="p-4">
              <h3 className="text-sm font-semibold text-foreground mb-1">{activeTab === "hoofd" ? "Hoofdmenu" : "Footer Menu"} Items</h3>
              <p className="text-xs text-muted-foreground mb-4">Sleep items of gebruik pijltjes om de volgorde aan te passen.</p>
            </div>
            <Separator />
            <div className="p-2">
              {currentMenu.map((item, i) => (
                <div key={item.id} className="flex items-center gap-3 px-3 py-2.5 rounded-md hover:bg-muted/30 group transition-colors">
        ...
        <Card>
          <div className="p-4">
            <h3 className="text-sm font-semibold text-foreground mb-4">Item Toevoegen</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Label</label>
                <Input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="Menu tekst" className="h-9" />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">URL</label>
                <Input value={newHref} onChange={(e) => setNewHref(e.target.value)} placeholder="/pagina-url" className="h-9 font-mono text-xs" />
              </div>
