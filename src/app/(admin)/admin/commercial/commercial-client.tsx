...
import {
  toggleCommercialArea as toggleCommercialAreaAction,
  createCommercialArea as createCommercialAreaAction,
  deleteCommercialArea as deleteCommercialAreaAction,
  updateCommercialArea as updateCommercialAreaAction,
...
const typeToDb: Record<string, "BANNER" | "POPUP" | "INLINE" | "SIDEBAR"> = {
  banner: "BANNER",
  popup: "POPUP",
  inline: "INLINE",
  sidebar: "SIDEBAR",
...
export default function CommercialClient({ initialAreas }: { initialAreas: CommercialArea[] }) {
  ...
  function handleToggleArea(id: string) {
    ...
    if (!area) return;
    const newEnabled = !area.enabled;
    setAreas(areas.map((a) => a.id === id ? { ...a, enabled: newEnabled } : a));
    startTransition(async () => {
      await toggleCommercialAreaAction(id, newEnabled);
    });
  }

  function handleAddArea() {
    startTransition(async () => {
      const created = await createCommercialAreaAction({
        name: "Nieuwe Zone",
        location: "Nog in te stellen",
        type: "BANNER",
        content: "",
        linkUrl: "",
        enabled: false,
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <Button onClick={handleAddArea} disabled={isPending} className="gap-1.5">
