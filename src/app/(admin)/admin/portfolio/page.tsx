...
const demoPortfolio: PortfolioItem[] = [
  { id: "pf1", title: "Rebranding Fernandes Groep", description: "Complete merkidentiteit vernieuwing inclusief logo, huisstijl en drukwerk.", category: "branding", mediaType: "slider", images: ["/placeholder-1.jpg", "/placeholder-2.jpg", "/placeholder-3.jpg"], status: "gepubliceerd" },
  { id: "pf2", title: "Event Merchandise SuriPop", description: "T-shirts, banners en promotional items voor SuriPop festival.", category: "evenement", mediaType: "foto", images: ["/placeholder-1.jpg"], status: "gepubliceerd" },
  { id: "pf3", title: "Voertuig Wrap - Taxi Fleet", description: "Volledige voertuigbelettering voor 15 taxi's in Paramaribo.", category: "signage", mediaType: "video", videoUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ", images: ["/placeholder-1.jpg"], status: "gepubliceerd" },
  { id: "pf4", title: "Packaging Design - SuriSpice", description: "Verpakkingsontwerp voor nieuwe kruidenlijn.", category: "verpakking", mediaType: "slider", images: ["/placeholder-1.jpg", "/placeholder-2.jpg"], status: "concept" },
  { id: "pf5", title: "Gevelreclame Restaurant Javaans", description: "LED gevelreclame en raambelettering.", category: "signage", mediaType: "foto", images: ["/placeholder-1.jpg"], status: "gepubliceerd" },
  { id: "pf6", title: "Social Media Campagne TeleSur", description: "Maandelijkse social media content creatie.", category: "digitaal", mediaType: "video", videoUrl: "https://www.youtube.com/watch?v=example", images: ["/placeholder-1.jpg"], status: "gepubliceerd" },
];
...
export default function PortfolioPage() {
  ...
  const [items, setItems] = useState(demoPortfolio);
  ...
  function toggleStatus(id: string) {
    setItems(items.map((i) => i.id === id ? { ...i, status: i.status === "gepubliceerd" ? "concept" as const : "gepubliceerd" as const } : i));
  }
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Button onClick={() => router.push("/admin/portfolio/nieuw")} className="gap-1.5">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${catFilter === "all" ? "bg-primary text-white" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
        >
          Alles ({items.length})
        </button>
        {portfolioCategories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${catFilter === cat ? "bg-primary text-white" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
            >
